...
export default {
  ...
  props: {
    product: {
      type: Object,
      default () { return {} } // 預設值先將product物件設為空物件
    }
  },
  data () {
    return {
      modal: {},
      tempProduct: {} // 外層的資料傳送的接收
    }
  },
  methods: {
    // showModal () {
    ...
    // },
    uploadFile () {
      const uploadedFile = this.$refs.fileInput.files[0] // 透過ref取得上傳的檔案
      console.dir(uploadedFile)
      const formData = new FormData()
      formData.append('file-to-upload', uploadedFile) // 增加一個欄位到這個表單裡(欄位名稱對應到name的'file-to-upload'，然後把剛剛取得的uploadedFile塞進那個欄位)
